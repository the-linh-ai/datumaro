...
from .format import (
    ...
    make_mapillary_instance_categories, parse_config_file,
)
...
class _MapillaryVistasExtractor(SourceExtractor):
    ...
    def _load_panoptic_config(path):
        panoptic_config_path = osp.join(path, MapillaryVistasPath.PANOPTIC_DIR,
            MapillaryVistasPath.PANOPTIC_CONFIG)
    ...
    def _load_instances_categories(self):
        ...
        if self._use_original_config:
            ...
            try:
                ...
                label_map = parse_config_file(osp.join(osp.dirname(self._path),
                                        config_file))
