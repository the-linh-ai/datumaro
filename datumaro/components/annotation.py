...
class Mask(Annotation):
    ...
    label: Optional[int] = field(validator=optional(instance_of(int)),
        default=None, kw_only=True)
    z_order: int = field(validator=instance_of(int), default=0, kw_only=True)

    _area: Optional[int] = field(default=None,
        validator=optional(instance_of((int, float))), kw_only=True, eq=False)
...
def _rounding_cmp(a, b):
    if COORDINATE_ROUNDING_DIGITS is not None:
        return np.allclose(a, b,
            rtol=0, atol=10 ** -COORDINATE_ROUNDING_DIGITS)
    else:
...
class _Shape(Annotation):
    ...
    points: List[float] = field(factory=list, validator=instance_of(list),
        eq=cmp_using(_rounding_cmp))
...
class Cuboid3d(Annotation):
    ...
    _points: List[float] = field(factory=lambda: [0, 0, 0,  0, 0, 0,  1, 1, 1],
        eq=cmp_using(_rounding_cmp))
...
class Polygon(_Shape):
    ...
    _area: Optional[int] = field(default=None,
        validator=optional(instance_of(int)), kw_only=True, eq=False)

    def __init__(self, points, *, label=None, z_order=None,
            id=None, group=None, attributes=None, area=None):
        assert isinstance(points, list)
        assert len(points) % 2 == 0 and 3 <= len(points) // 2, \
            "Wrong polygon points: %s" % points
        self.points = points
